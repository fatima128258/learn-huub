...
// POST - Submit activity file for a student
export async function POST(request) {
  try {
    ...
    // Verify that the content at this order is an activity
    const contentItem = playlist.content?.find(item => item.order === contentOrder);
    if (!contentItem || contentItem.type !== "activity") {
      return NextResponse.json(
        { message: "Content at this order is not an activity" },
        { status: 400 }
      );
    }
    ...
    activityProgressMap.set(contentOrder.toString(), {
    ...
    const activityProgressObj = {};
    activityProgressMap.forEach((value, key) => {
      activityProgressObj[key] = {
